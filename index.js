...
(function() {
    ...
    // Add TTS button to message
    function addTTSButtonToMessage(messageElement) {
        // Check if button already exists
        if ($(messageElement).find(".gemini-tts-message-btn").length > 0) return;
        
        const mesBlock = $(messageElement).closest(".mes");
        if (!mesBlock.length) return;
        
        const extraActions = mesBlock.find(".extraMesButtons, .mes_buttons");
        if (!extraActions.length) return;
        
        // Create TTS button with speaking emoji
        const ttsButton = $(`
            <div class="mes_button gemini-tts-message-btn" title="Play with Gemini TTS">
                <i class="fa-solid fa-face-smile" style="font-size: 16px;"></i>
            </div>
        `);
        ...
        ttsButton.on("click", async function(e) {
            ...
            const characterId = mesBlock.attr("ch_name") || mesBlock.attr("data-ch-name");
            if (!characterId) {
                console.error("[Gemini TTS Plus] Could not determine character for message");
                return;
            }
        ...
        extraActions.prepend(ttsButton);
    }
    ...
    function observeMessages() {
        ...
        document.querySelectorAll(".mes").forEach(addTTSButtonToMessage);
    }
...
3. **If it describes a Physical Action (e.g., [chewing slowly, kissing]):**
   -> Perform the action *while* speaking or in between words (e.g., speak with your mouth full, or breathe heavily).`;
        ...
        for (const char of characters) {
            ...
            const currentInstructions = settings.characterInstructions[charId] || defaultInstructions;
            const currentPersonality = settings.characterPersonality[charId] || "";
            const currentVocalTraits = settings.characterVocalTraits[charId] || "";
            ...
            html += `
                <div class="gemini-tts-char-item">
                    ...
                    <div class="gemini-tts-char-controls">
                        ...
                        <textarea class="gemini-tts-instructions-input" data-char-id="${charId}" 
                            placeholder="TTS prompting instructions..."
                            rows="8">${currentInstructions}</textarea>
                        
                        <label>Character Personality:</label>
                        <textarea class="gemini-tts-personality-input" data-char-id="${charId}" 
                        ...
                        <textarea class="gemini-tts-vocal-input" data-char-id="${charId}" 
