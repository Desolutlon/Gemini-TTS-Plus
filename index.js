...
(function() {
    ...
    function registerMessageAction() {
        ...
        if (!context) {
            console.error("[Gemini TTS Plus] Could not get SillyTavern context");
            return;
        }
        
        // Register using SillyTavern's messageActions API
        if (window.SillyTavern?.messageActions) {
            window.SillyTavern.messageActions.register(
                'gemini-tts-play',
                'Play TTS',
            ...
            console.log("[Gemini TTS Plus] Message action registered");
        } else {
            console.error("[Gemini TTS Plus] messageActions API not available");
        }
    }
    
    // Observer to add TTS buttons to new messages
    function observeMessages() {
        // No longer needed since we're using messageActions API
        console.log("[Gemini TTS Plus] Using messageActions API, no observer needed");
    }
    ...
    jQuery(async () => {
        try {
            ...
            if (window.SillyTavern?.getContext) {
                ...
                // Hook into message events for auto-play
