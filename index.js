...
(function() {
    ...
    function addTTSButtonToMessage(messageElement) {
        ...
        const mesBlock = $(messageElement);
        if (!mesBlock.hasClass("mes")) return;
        
        // Find the message actions menu (three dots menu)
        const mesButtonsContainer = mesBlock.find(".mes_buttons");
        if (!mesButtonsContainer.length) return;
        ...
        ttsButton.on("click", async function(e) {
            ...
            // Get character name from the message
        ...
        // Insert button into the message actions
        mesButtonsContainer.prepend(ttsButton);
    }
